...
class Patch_Temporal_Attention(nn.Module):
    def __init__(self, embed_size, heads, time_num, dropout,stride,patch_len,**kwargs):
        super(Patch_Temporal_Attention, self).__init__()
        ...
        patch_num = int(( out_len- patch_len) /stride + 1) + 1  # 根据公式直接计算
...
class ST_Blcok(nn.Module):
    def __init__(self, c_in, c_out, num_nodes, tem_size,
                 Kt, dropout, support_len=3,**kwargs):
        super(ST_Blcok, self).__init__()
        ...
        self.multigcn = Gate_GCN(c_out,2 * c_out,dropout=dropout,support_len=support_len,Kt=Kt,nhid=args.d_model,num_gcn=args.num_gcn)
        self.num_nodes = num_nodes
        self.tem_size = tem_size
        time_num=24*60/args.points_per_hour

        out_len=tem_size  # According to the formula for Dilated Causal Convolution
        # Patch_Temporal_Attention
        self.TAT = Patch_Temporal_Attention(c_out,heads=args.heads,time_num=time_num,dropout=dropout,stride=args.stride,patch_len=args.patch_len,out_len=out_len)
        self.c_out = c_out
...
class Adaptive_Multi_Head_GCN(nn.Module):
    def __init__(self, in_features, out_features, bias=True,num=10):
        '''num: the num of heads in Adaptive_Multi_Head_GCN'''
        super(Adaptive_Multi_Head_GCN, self).__init__()
...
class Gate_GCN(nn.Module):
    def __init__(self, dim_in,dim_out,Kt=2,dropout=0.0,support_len=2,nhid=64,num_gcn =10):
        super(Gate_GCN, self).__init__()

        self.gc1=nn.ModuleList()
        self.gc2=nn.ModuleList()
        for i in range(support_len):
            self.gc1.append(Adaptive_Multi_Head_GCN(dim_in, nhid,num=num_gcn))
            self.gc2.append(Adaptive_Multi_Head_GCN(nhid, dim_out,num=num_gcn))
